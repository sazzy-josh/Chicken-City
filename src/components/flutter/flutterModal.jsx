        ...
        className='inner_Flutter_Modal h-[44vh] w-[85vw]  sm:w-[45vw] sm:h-[50vh] lg:w-[30vw] lg:h-[50vh]  bg-slate-300 rounded-xl shadow-xl text-white p-4 flex flex-col'>
         <div className='flex justify-between h-1/4'> <div className='flex'><div className='rounded-full w-12 h-12 p-1 bg-slate-800 flex items-center justify-center'><img src={logo} alt="flutter" className='sm:w-8 sm:h-8'/></div> </div> <span  className='w-12 p-1 h-12 cursor-pointer flex justify-center items-center' onClick={flutterModal}><MdCancel className="w-8 h-8 text-red-500" /> </span> </div>
           ...
           <form className='flex flex-col  text-black gap-y-2'> 
            <input type="text" name="fullName" placeholder='Idahosa Osaze' className='p-2 py-3 rounded-md outline-none  text-xs placeholder-black placeholder:opacity-[30%]' required onChange={handleName} value={name} />
            <input type="email" name="email" placeholder='idahosajoshua61@gmail.com' className='p-2 py-3 rounded-md  outline-none  text-xs placeholder-black placeholder:opacity-[30%]' required onChange={(e)=> {setEmail(e.target.value)}} value={email}/>
            <input type="tel" name="phone" placeholder="+234-700-000-0000" className='p-2 rounded-md py-3 outline-none  text-xs placeholder-black placeholder:opacity-[30%]' required onChange={(e)=> {setPhone(e.target.value)}} value={phone} />
